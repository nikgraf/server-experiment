(function() {
  ...
  function resolve(base, name) {
    ...
    while ((m = rel.exec(name))) {
      if (m[1]) {
        var end = /\/([^\/]+)\/$/.exec(base);
        if (end && end[1] != "..")
          base = base.slice(0, base.length - end[0].length + 1);
        else base += "../";
      }
  ...
  function Module(path, base) {
    ...
    this.require = function(name) {
      ...
      var resp = get(
        base + name.replace(/(^|\/)\.\.(?=$|\/)/g, "$1__").replace(/\.js$/, "")
      );
      var resolvedName = resp.url
        .match(/.*?\/moduleserve\/mod(\/.*)/)[1]
        .replace(/(^|\/)__(?=$|\/)/g, "$1..");
  ...
  function evalFunction(content, name) {
    var prefix = "(function(module, require, exports){",
      suffix = "\n})";
  ...
  var script =
    document.currentScript || document.querySelector("script[data-module]");
