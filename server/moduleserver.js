...
ModuleServer.prototype.handleRequest = function(req, resp) {
  ...
  var send = function(status, text, headers) {
    var hds = { "access-control-allow-origin": "*", "x-request-url": req.url };
    if (!headers || typeof headers == "string")
      hds["content-type"] = headers || "text/plain";
    else for (var prop in headers) hds[prop] = headers[prop];
  ...
  if (found != path) {
    ...
    send(301, null, {
      location: "/moduleserve/mod/" + dash(found.slice(0, found.length - 3))
    });
    return true;
  }
...
function undash(path) {
  return path.replace(/(^|\/)__(?=$|\/)/g, "$1..");
}
function dash(path) {
  return path.replace(/(^|\/)\.\.(?=$|\/)/g, "$1__");
}

function resolve() {
  return unwin(pth.resolve.apply(pth, arguments));
}
function relative() {
  return unwin(pth.relative.apply(pth, arguments));
}
var unwin =
  pth.sep == "\\"
    ? function(s) {
...
ModuleServer.prototype.resolveModule = function(path) {
  ...
  var hasMod = localPath.indexOf("/__mod/"),
    parent,
    modPath,
    resolved;
  if (hasMod > -1) {
  ...
  try {
    resolved = unwin(module_._resolveFilename(modPath, dummyMod));
  } catch (e) {
    return null;
  }
...
ModuleServer.prototype.sendScript = function(path, send) {
  var content,
    localPath = resolve(this.root, path);
  try {
